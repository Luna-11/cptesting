...
export default function CalendarApp() {
  ...
  const handleDateClick = (info: any) => {
    // Log the info object to see its structure for debugging
    console.log(info);

    if (!info || !info.dateStr) {
      console.error("Invalid info object or dateStr");
      return;
    }
    ...
    if (title && title.trim() !== "") {
      ...
      alert("No title entered or empty title. Event not added.");
    }
  };
  ...
  const handleEventClick = (info: any) => {
    if (info && info.event && info.event.title) {
      ...
      console.error("Invalid event info or missing event title.");
    }
  };
