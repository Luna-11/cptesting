  ...
  try {
    ...
    if (data.username) {
      ...
      if (currentUsername !== data.username) {
        await connection.execute(`UPDATE user SET name = ? WHERE user_id = ?`, [data.username, userId])
      }
    }
    ...
    if (profiles.length > 0) {
      ...
      if (updateFields.length > 0) {
        ...
        await connection.execute(`UPDATE user_profiles SET ${updateFields.join(", ")} WHERE user_id = ?`, updateValues)
      }
    } else {
      // Create new profile
